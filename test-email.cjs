  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px; background: #f9fafb; border-radius: 12px;">
      <div style="text-align: center; margin-bottom: 24px;">
        <h2 style="margin: 0; color: #111827; font-size: 20px;">🧺 I&C Laundry</h2>
      </div>
      <div style="background: #fff; border-radius: 10px; padding: 24px; border: 1px solid #e5e7eb;">
        <h3 style="margin: 0 0 16px; color: #111827; font-size: 16px;">Order Received! (Order #IC-TEST-0001)</h3>
        <div style="color: #374151; font-size: 14px; line-height: 1.6;">${lines}</div>
      </div>
      <div style="text-align: center; margin-top: 20px; color: #9ca3af; font-size: 12px;">
        <p>This is an automated notification from I&C Laundry Management System</p>
      </div>
    </div>`;

  try {
    const info = await transporter.sendMail({
      from: `"I&C Laundry" <${GMAIL_EMAIL}>`,
      replyTo: GMAIL_EMAIL,
      to: testTo,
      subject: "Order Received! (Order #IC-TEST-0001)",
      text: body,
      html,
    });
